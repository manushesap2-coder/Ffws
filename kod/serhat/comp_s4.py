...
from tl_s4 import W, H, FPS, FPS_SRC, T_DROP, T_VO, T_END, B
...
def init_gfx():
    ...
    SPR['proj_lbl'] = chip_spr('star', 'İNCİ PROJESİ', 34, dark=0.82)
...
def draw_callouts(frame, T, info, M):
    ...
    if sh == 'YOL':
        ...
        CO.ring(frame, P[0], T, 44.40 + TL.EXT, fo, r0=10, r1=46)
        CO.pin(frame, P[0], T, 44.18 + TL.EXT, t1 - 0.04)
        CO.show_tag(frame, SPR['t_mey'], (P[0][0], P[0][1] - 66), T, 44.52 + TL.EXT, t1 - 0.04)
        pb = ease_io(np.clip((T - 45.18 - TL.EXT) / 0.42, 0, 1))
        if pb > 0: CO.glow_path(frame, CO.partial(P[1:], False, pb), fo, width=6, head=pb < 1)
        bm = (P[1] + P[2]) / 2
        CO.show_tag(frame, SPR['t_bel'], (bm[0], bm[1] + 16), T, 45.58 + TL.EXT, t1 - 0.04)

# ---------------------------------------------------------------- v4: project pin (İnci building) in PARK / MEY / WIDE
B2 = {}
for _nm, _fs in (('D_B', ('b2_bwd_B', 'b2_fwd_B')), ('D_C', ('b2_fwd_C',))):
    _d = {}
    for _f in _fs: _d.update({int(k): v for k, v in json.load(open(f'/tmp/jobs/serhat/v4/{_f}.json'))['pts'].items()})
    B2[_nm] = _d
def proj_pt(info):
    d = B2.get(info.get('name'))
    if not d: return None
    k = info['lt'] * FPS_SRC; k0 = int(np.floor(k)); a = k - k0
    if k0 < min(d) or k0 + 1 > max(d): return None
    P = np.array(d[k0][:2]) * (1 - a) + np.array(d[k0 + 1][:2]) * a
    s = info['src']                                  # LK drifts off the concrete slab after ~src 75; hand-measured anchors pull it back
    return P + np.array([np.interp(s, CORR_S, CORR_X), np.interp(s, CORR_S, CORR_Y)])
CORR_S = [0, 70, 80, 90, 95, 101, 107, 113, 116, 118, 119.8, 135.6, 138.5, 200]
CORR_X = [0, 0, 8, 2, 11, 11, 43, 82, 104, 116, 133, 169, 170, 170]
CORR_Y = [0, 0, -43, -43, -45, -57, -62, -74, -74, -74, -70, -42, -38, -38]
PROJ_WIN = dict(PARK=(43.30, False), MEY=(None, True), WIDE=(None, True))   # (t_in or None = shot start, label below pin)
def draw_proj(frame, T, info, M):
    sh = info['shot']
    if sh not in PROJ_WIN: return
    S_ = next(s for s in TL.SHOTS if s['n'] == sh)
    t_in, below = PROJ_WIN[sh]; t_in = t_in if t_in is not None else S_['t0'] + 0.12
    t_out = S_['t1'] - 0.04
    if not (t_in <= T <= t_out): return
    P = proj_pt(info)
    if P is None: return
    P = to_out(M, P[None])[0]
    q = float(np.clip((t_out - T) / 0.18, 0, 1)); p = float(np.clip((T - t_in) / 0.40, 0, 1))
    drop = (1 - ease_out_back(p, 2.0)) * 200
    ring_t = (T - t_in - 0.3) % 1.1
    if T > t_in + 0.3:
        r = 16 + 56 * ease_out(ring_t / 1.1, 2); a = (1 - ring_t / 1.1) * 0.9 * q
    ...
    lb = SPR['proj_lbl']; lh, lw = lb.shape[:2]
    lx = float(np.clip(P[0] - lw / 2, -20, W - lw + 20))
    ly = P[1] + 26 + lh / 2 if below else P[1] - h - lh / 2 - 6 - drop
    slide_in(frame, lb, lx, ly, T, t_in + 0.28, t_out, dist=0)
...
def draw_chips(frame, T):
    ...
    g1, g2 = TL.vo('güçlü')['T'] - 0.1, TL.vo('değerli')['T'] - 0.1; e4 = B(61) + TL.EXT - 0.12
...
def draw_finale(frame, T):
    ...
    # v4: no fade to black here; the Şule clip is cross-dissolved in after T_FIN
...
def render_frame(fi):
    ...
    if info['shot'] in PROJ_WIN: draw_proj(frame, T, info, M)
...
def main():
    ...
    ap.add_argument('--outdir', default='/tmp/jobs/serhat/out/frames4')
