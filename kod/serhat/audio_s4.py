...
k = int(round(TL.VO_SPLIT * SR)); f5 = int(0.005 * SR)
v[k - f5:k] *= np.linspace(1, 0, f5, dtype=np.float32); v[k:k + f5] *= np.linspace(0, 1, f5, dtype=np.float32)
v = np.concatenate([v[:k], np.zeros(int(round(TL.VO_PAUSE * SR)), np.float32), v[k:]])
put(mix_v, v, TL.T_VO)
# ---- v4: Şule's CTA after the finale (same voice chain as the girl)
_al = json.load(open(W + 's83_align.json'))
sv = load(W + 's83_df48.wav', 'highpass=f=90,equalizer=f=250:t=q:w=1.0:g=-2.5,equalizer=f=3500:t=q:w=0.9:g=2.5,equalizer=f=9000:t=q:w=1.0:g=1.0,'
          'deesser=i=0.35,acompressor=threshold=-24dB:ratio=3:attack=5:release=90:makeup=2')
sv = sv[int(TL.SULE_S0 * SR):int((_al[-1]['e'] + 0.40) * SR)].copy(); fl = int(0.25 * SR); sv[-fl:] *= np.linspace(1, 0, fl)
sv *= db(-20 - rms_active(sv))
put(mix_v, sv, TL.T_SULE)
...
parts = [seg(t0 + BAR, t0 + 4 * BAR), seg(t0, 15.595), seg(15.595, 15.595 + TL.T_TOT - TL.T_DROP + 1.0)]
...
duck = -12.5 * act
gain_db = base + duck
fo = (t >= TL.T_TOT - 2.4); gain_db[fo] -= 40 * ((t[fo] - (TL.T_TOT - 2.4)) / 2.4) ** 2
...
B = TL.B
cuts = [(B(18), 1492), (B(25), 1490), (B(42), 1492), (B(47) + TL.EXT, 1490), (B(53) + TL.EXT, 1492), (B(61) + TL.EXT, 1492), (TL.T_SULE + TL.XF / 2, 1490)]
peaks = {1490: 0.73, 1492: 1.08, 3115: 0.11}
for tc, i in cuts: sfx(i, tc, peaks[i], -11)
pops = [TL.vo('çarşının')['T'] + 0.2, 30.65, 31.83, TL.vo('Geniş')['T'], TL.vo('yüksek')['T'], TL.vo('asma')['T'],
        TL.vo('farklı')['T'], TL.vo('ferah')['T'], 42.39, 43.0, 44.55 + TL.EXT, 45.61 + TL.EXT,
        TL.vo('güçlü')['T'], TL.vo('değerli')['T'], TL.vo('Serhat')['T'], TL.vo('İnci')['T']]
for tp in pops: sfx(3005, tp, 0.03, -19)
for t_line in (30.28, 31.22, 41.80, 42.72, 45.18 + TL.EXT): sfx(1491, t_line + 0.22, 0.30, -21)     # callout lines drawing
sfx(3005, 44.45 + TL.EXT, 0.03, -16)
_sh = {x['n']: x for x in TL.SHOTS}
for tp in (43.30 + 0.35, _sh['MEY']['t0'] + 0.47, _sh['WIDE']['t0'] + 0.47): sfx(3005, tp, 0.03, -16)   # v4 project pins land
sfx(2350, TL.T_SULE + (_al[18]['s'] - TL.SULE_S0) + 0.15, 0.45, -14)                                  # end-card logo sparkle                                                             # pin lands
...
mix = mix[:int(TL.T_TOT * SR)]
sf.write('/tmp/jobs/serhat/out/mix4_raw.wav', mix, SR, subtype='FLOAT')
# stems for QC
for nm, b in (('voice', mix_v), ('music', mix_m), ('sfx', mix_s)): sf.write(f'/tmp/jobs/serhat/out/stem4_{nm}.wav', b[:len(mix)], SR, subtype='FLOAT')
