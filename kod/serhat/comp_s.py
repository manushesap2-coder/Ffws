...
def render_plate(T):
    ...
    if sh['kind'] == 'take':
        ...
        z, cx, cy = take_zoom(T)
        k = np.clip((T - (T_DROP - 0.25)) / 0.7, 0, 1)
        g = mix_grade(G_INT, G_EXT, float(k))
...
def finish(img, T):
    ...
    img = img * _VIG
    gr = np.random.default_rng(int(T * 1000)).normal(0, 0.008, (H // 2, W // 2)).astype(np.float32)
...
def main():
    ...
    ap.add_argument('--outdir', default='/tmp/jobs/serhat/out/frames')
