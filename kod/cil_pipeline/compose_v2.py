...
SHOTS_T = [
    ...
    dict(n='A2', src='SPK_A', mode='spk', t0=3.080, t1=B(0), z0=2.42, z1=2.56, grade=G_SPK, tin=('punch', 6)),
    ...
    dict(n='B2', src='B2', mode='broll', t0=B(2), t1=B(4), src_in=0.40, speed=1.15, z0=1.08, z1=1.14, grade=drn(sky=0.5), tin=('whip', 6, (-1, 0))),
    dict(n='B3', src='B1', mode='broll', t0=B(4), t1=B(5), src_in=2.20, speed=1.0, z0=1.10, z1=1.14, grade=drn(sky=0.7), tin=('zoom', 5)),
    dict(n='C1', src='C1', mode='broll', t0=B(5), t1=B(7), src_in=0.30, speed=1.0, z0=1.06, z1=1.12, grade=drn(sky=0.5), tin=('whip', 6, (0, -1))),
    dict(n='C2', src='C2', mode='broll', t0=B(7), t1=B(9), src_in=0.30, speed=1.0, z0=1.08, z1=1.13, grade=drn(sky=0.8), tin=('spin', 7, 1)),
    dict(n='C3', src='C3', mode='broll', t0=B(9), t1=B(11), src_in=0.30, speed=1.0, z0=1.06, z1=1.12, grade=drn(sky=0.5), tin=('whip', 6, (1, 0))),
    dict(n='C4', src='C4', mode='broll', t0=B(11), t1=B(14), src_in=0.20, speed=1.0, z0=1.04, z1=1.12, grade=drn(), tin=('glitch', 6)),
    dict(n='D1', src='D1', mode='broll', t0=B(14), t1=B(17), src_in=0.90, speed=1.0, z0=1.04, z1=1.10, grade=drn(), tin=('whip', 6, (-1, 0))),
    dict(n='D2', src='D2', mode='broll', t0=B(17), t1=B(21), src_in=0.50, speed=1.5, z0=1.10, z1=1.18, grade=drn(), tin=('spin', 7, 1), outline=True),
    dict(n='E1', src='SPK_E', mode='spk', t0=B(21), t1=B(23), z0=2.12, z1=2.24, grade=G_SPK, tin=('whip', 6, (1, 0))),
    dict(n='E2', src='E2', mode='broll', t0=B(23), t1=B(28), src_in=4.0, ramp=[(0.0, 3.2), (0.55, 3.0), (1.05, 0.9), (2.42, 0.55)], z0=1.04, z1=1.16, grade=drn(), tin=('zoomout', 7)),
    dict(n='F1', src='F1', mode='broll', t0=B(28), t1=B(29), src_in=0.50, speed=1.5, z0=1.10, z1=1.14, grade=drn(), tin=('flashzoom', 7), shake=(0.0, 12, 14)),
    dict(n='F2', src='F2', mode='broll', t0=B(29), t1=B(30), src_in=0.30, speed=1.2, z0=1.12, z1=1.16, grade=drn(sky=0.6), tin=('whip', 5, (-1, 0))),
    dict(n='F3', src='F3', mode='broll', t0=B(30), t1=B(31), src_in=0.30, speed=1.3, z0=1.08, z1=1.12, grade=drn(), tin=('whip', 5, (1, 0))),
    dict(n='F4', src='F4', mode='broll', t0=B(31), t1=B(32), src_in=0.30, speed=1.2, z0=1.10, z1=1.14, grade=drn(sky=0.5), tin=('whip', 5, (-1, 0))),
    dict(n='F5', src='F5', mode='broll', t0=B(32), t1=B(34), src_in=0.20, ramp=[(0.0, 2.0), (0.4, 5.5), (0.97, 5.5)], z0=1.10, z1=1.02, grade=drn(), tin=('whip', 6, (0, -1))),
    dict(n='F6', src='F6', mode='broll', t0=B(34), t1=B(36), src_in=0.50, ramp=[(0.0, 3.0), (0.97, 1.4)], z0=1.14, z1=1.06, grade=drn(), tin=('spin', 7, -1)),
...
def trans_params(sh, T, nxt):
    ...
    if tin and kin < tin[1]:
        ...
        p = 1 - kin / n; e = p ** 2
        if typ == 'whip':
            d = tin[2]; fx['off'] = [-d[0] * e * 0.62 * W, -d[1] * e * 0.62 * H]
            fx['blur_dir'] = (d[0] * e * 240, d[1] * e * 240); fx['rgb'] = 14 * e
        elif typ == 'zoom':
            fx['extra_scale'] = 1 + 0.40 * e; fx['radial'] = 0.16 * e; fx['rgb'] = 10 * e
        elif typ == 'zoomout':
            fx['extra_scale'] = 1 - 0.26 * e; fx['radial'] = 0.12 * e
        elif typ == 'spin':
            fx['rot'] = -tin[2] * 40 * e; fx['extra_scale'] = 1 + 0.5 * e; fx['spin'] = -tin[2] * 22 * e; fx['rgb'] = 8 * e
        elif typ == 'punch':
            fx['extra_scale'] = 1 + 0.15 * e
        elif typ == 'glitch':
            fx['glitch'] = e; fx['rgb'] = 22 * e; fx['extra_scale'] = 1 + 0.06 * e
        elif typ == 'pushin':
            fx['extra_scale'] = 1 - 0.30 * e; fx['radial'] = 0.10 * e
        elif typ in ('flashzoom', 'flash'):
            fx['flash'] = 0.95 * p ** 1.6
            if typ == 'flashzoom':
                fx['extra_scale'] = 1 + 0.30 * e; fx['radial'] = 0.13 * e; fx['leak'] = p
    if nxt and nxt.get('tin'):
        ...
        if 0 < kout <= n_out:
            ...
            if typ == 'whip':
                d = nxt['tin'][2]; fx['off'] = [d[0] * e * 0.62 * W, d[1] * e * 0.62 * H]
                fx['blur_dir'] = (d[0] * e * 240, d[1] * e * 240); fx['rgb'] = 14 * e
            elif typ in ('zoom', 'flashzoom'):
                fx['extra_scale'] *= 1 + 0.34 * e; fx['radial'] = max(fx['radial'], 0.15 * e)
            elif typ == 'zoomout':
                fx['extra_scale'] *= 1 - 0.12 * e; fx['radial'] = max(fx['radial'], 0.07 * e)
            elif typ == 'spin':
                fx['rot'] += nxt['tin'][2] * 40 * e; fx['extra_scale'] *= 1 + 0.5 * e; fx['spin'] = nxt['tin'][2] * 22 * e
            elif typ == 'flash':
                fx['flash'] = max(fx['flash'], 0.7 * e); fx['extra_scale'] *= 1 + 0.08 * e
            elif typ == 'glitch':
                fx['glitch'] = e; fx['rgb'] = 18 * e
    return fx
...
def draw_outline(img, T, M, sh, ts):
    ...
    gold = np.array([0.40, 0.98, 0.70], np.float32)
    if fill_a > 0:
    ...
    img[:] = np.clip(img * (1 - c) + np.array([0.93, 1.0, 0.96], np.float32) * c, 0, 1)
...
def render_shot(sh, T, nxt):
    ...
    if sh['mode'] == 'spk':
        ...
        tr = T - sh['t0']
        ts, spd = src_time(sh, tr)
        if 'ramp' not in sh:
            burst = 1.6 * 0.12 * (1 - np.exp(-tr / 0.12))      # fast start that settles: speed x(1+1.6e^-t/0.12)
            ts += sh['speed'] * burst; spd = sh['speed'] * (1 + 1.6 * np.exp(-tr / 0.12))
    ...
    if fx.get('glitch', 0) > 0.05: img = glitch(img, fx['glitch'], T)
    if fx.get('rgb', 0) > 0.8: img = rgb_split(img, fx['rgb'])
    if fx.get('leak', 0) > 0: img = light_leak(img, fx['leak'], T)
    return img

def beat_pulse(T, sh):
    k = np.floor((T - TL.T_DROP1) / BEAT); tb = TL.T_DROP1 + k * BEAT
    if tb < sh['t0'] + 0.05 or int(k) % 2: return 1.0
    return 1.0 + 0.035 * np.exp(-(T - tb) / 0.10)

def rgb_split(img, px):
    px = int(round(px)); out = img.copy()
    out[..., 0] = np.roll(img[..., 0], px, 1); out[..., 2] = np.roll(img[..., 2], -px, 1)
    return out

def glitch(img, e, T):
    rng = np.random.default_rng(int(T * 1000)); out = img.copy()
    for _ in range(int(3 + 6 * e)):
        y0 = int(rng.uniform(0, H - 40)); hh = int(rng.uniform(12, 90)); dx = int(rng.uniform(-80, 80) * e)
        out[y0:y0 + hh] = np.roll(img[y0:y0 + hh], dx, 1)
    return out

_LEAK = None
def light_leak(img, p, T):
    global _LEAK
    if _LEAK is None:
        yy, xx = np.mgrid[0:H // 4, 0:W // 4].astype(np.float32)
        _LEAK = (yy, xx)
    yy, xx = _LEAK; cx = (0.1 + 0.9 * (1 - p)) * W / 4; cy = (0.25 + 0.4 * (1 - p)) * H / 4
    g = np.exp(-(((xx - cx) / (W / 7)) ** 2 + ((yy - cy) / (H / 6)) ** 2))
    g = cv2.resize(g, (W, H))[..., None] * (0.55 * np.sin(np.pi * p))
    col = np.array([1.0, 0.90, 0.70], np.float32) * 0.6 + MINT * 0.4
    return np.clip(img + g * col, 0, 1)
...
def draw_captions(frame, T, phrases):
    for items, start, end in phrases:
        ...
        draw_caption_pill(frame, T, items, start, end, CAP_Y); continue
...
def init_gfx():
    ...
    G['bt1'] = block_title('VİSTA', 'KURTULUŞ', 138, 64, 8, 22)
    G['bt2'] = block_title('GROBETON', 'DÖKÜMÜ', 116, 74, 6, 20)
    G['emcard'] = em_card(ren, 760)
    G['panel'] = panel_sprite(860, 500)
    G['hdr2'] = solid(text_mask('HER AŞAMADA', 'Montserrat-Bold.ttf', 44, 14), GOLDL)
    G['items2'] = [solid(text_mask(t, 'Montserrat-ExtraBold.ttf', 64, 1), WH) for t in ['SAĞLAMLIK', 'KALİTE', 'DOĞRU UYGULAMA']]
    G['brand2'] = shadowed(solid(text_mask('ÇİL', 'Montserrat-Black.ttf', 170, 14), CREAM), blur=10, off=(0, 6), strength=0.5, pad=24)
    G['sub2'] = solid(text_mask('YAPI & İNŞAAT', 'Montserrat-SemiBold.ttf', 52, 16), MINT)
    G['project2'] = solid(text_mask('VİSTA  |  I  ·  KURTULUŞ', 'Montserrat-Medium.ttf', 38, 6), CREAM)
...
def draw_checklist(frame, T):
    return draw_checklist_v2(frame, T)

def draw_checklist_v2(frame, T):
    t_in, t_out = wT('Her')['T'] - 0.10, B(14) - 0.03
    if not (t_in <= T < t_out): return
    u = T - t_in; p = ease_out(np.clip(u / 0.28, 0, 1), 3); q = np.clip((t_out - T) / 0.2, 0, 1)
    ps = G['panel']; ph, pw = ps.shape[:2]; cx = W / 2 - (1 - p) * 700 + (1 - q) * 700; cy0 = 860
    place(frame, ps, cx, cy0, opacity=float(min(1, p * 1.5) * q))
    hd = G['hdr2']; place(frame, hd, cx, cy0 - 170, opacity=float(p * q))
    times = [wT('sağlamlık')['T'], wT('kalite')['T'], wT('doğru')['T']]
    for i, (t_i, spr) in enumerate(zip(times, G['items2'])):
        if T < t_i - 0.05: continue
        ki = T - (t_i - 0.05); pi = ease_out(np.clip(ki / 0.24, 0, 1), 3); y = cy0 - 60 + i * 112
        place(frame, check_badge(78, np.clip(ki / 0.26, 0, 1)), cx - 330, y, scale=0.6 + 0.4 * ease_out_back(np.clip(ki / 0.22, 0, 1), 2.4), opacity=float(q))
        place(frame, spr, cx - 270 + spr.shape[1] / 2 + 90 * (1 - pi), y + 3, opacity=float(pi * q))

def draw_checklist_v1(frame, T):
    t_in, t_out = wT('Her')['T'] - 0.10, B(14) - 0.03
    if not (t_in <= T < t_out): return
    k = (T - t_in) * FPS
...
def draw_endcard(frame, T):
    return draw_endcard_v2(frame, T)

def draw_endcard_v2(frame, T):
    t = T - B(36)
    frame[:] = G['bg']
    am_ = np.zeros(frame.shape[:2], np.float32)
    ...
    frame[:] = frame * (1 - am_[..., None]) + MINT * am_[..., None]
    an, am = G['anchor']
    pa = np.clip(t / 0.5, 0, 1); sc = 0.7 + 0.3 * ease_out_back(pa, 1.8)
    s_an = shine(an, am, -0.25 + 1.5 * (t - 0.5) / 0.9, width=0.10, strength=0.85) if 0.5 <= t <= 1.4 else an
    hit = 4 * BEAT; pulse = float(np.exp(-((t - hit) / 0.12) ** 2))
    place(frame, s_an, W / 2, 690, scale=sc * (1 + 0.02 * pulse), opacity=min(1, t / 0.15), blur=6 * (1 - pa))
    for key, cy, td in [('brand2', 1122, 0.28), ('sub2', 1252, 0.46), ('project2', 1395, hit - 0.05)]:
        spr = G[key]; pt = np.clip((t - td) / 0.36, 0, 1)
        if pt > 0: place(frame, spr, W / 2 + 60 * (1 - ease_out(pt)) * (1 if key != 'sub2' else -1), cy, opacity=ease_out(pt), blur=4 * (1 - pt))
    pl = np.clip((t - (hit - 0.25)) / 0.35, 0, 1)
    if pl > 0:
        half = 190 * ease_out(pl)
        cv2.line(frame, (int(W / 2 - half), 1330), (int(W / 2 + half), 1330), tuple(float(c) for c in GOLDL), 2, cv2.LINE_AA)
    fo = np.clip((T - (T_END - 0.55)) / 0.55, 0, 1)
    if fo > 0: frame *= (1 - fo)

def draw_endcard_v1(frame, T):
    t = T - B(36)
    frame[:] = G['bg']
...
def render_frame(fi, phrases):
    ...
    draw_block_title(frame, T, G['bt1'], wT('Vista')['T'] - 0.10, 1.30, 330)
    draw_em_card(frame, G['emcard'], T, 1.36, 3.02, cy=320)
    draw_block_title(frame, T, G['bt2'], B(0), B(4) - 0.22, 700, slam=True)
...
def main():
    ...
    ap.add_argument('--out', type=str, default='/tmp/edit/out/v2_nosound.mp4')
    ap.add_argument('--start', type=float, default=0.0)
    ap.add_argument('--end', type=float, default=None)
    a = ap.parse_args()
    TRACK = build_track(); init_gfx(); phrases = build_phrases()
    if a.frames:
        os.makedirs('/tmp/edit/out/frames_v2', exist_ok=True)
        for f in sorted(int(round(float(x) * FPS)) for x in a.frames.split(',')):
            img = render_frame(f, phrases)
            cv2.imwrite(f'/tmp/edit/out/frames_v2/f_{f:04d}.png', (img[..., ::-1] * 255 + 0.5).astype(np.uint8))
